                    ...
                    @for($i = 0; $i<$date_count;$i++)
                        ...
                        @else
                        {{ Form::open(['method'=>'get', 'route' => ['orders.edit', $store->id]]) }}    
                    ...
                    @foreach($products as $product)
                        ...
                        <td>{{ Form::open(['method'=>'get', 'route' => ['orders.edit', $store->id]]) }}    
                        {{Form::hidden('start',$start->format('Y-m-d')) }}
                        {{Form::hidden('end',$end->format('Y-m-d')) }}
                   ...
                   <div class="">
                {{Form::open(['method' => 'post', 'route' => ['orders.store'], 'class' => 'row justify-content-around  my-5']) }}  

                    {!! Form::submit('承認して確定', ['class' => 'btn btn-outline-dark col-4']) !!}
                    {{Form::hidden('start',$start->format('Y-m-d')) }}
                    {{Form::hidden('end',$end->format('Y-m-d')) }}
