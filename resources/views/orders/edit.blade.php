...
@section('content')
     @if (Auth::check())
        ...
        @switch (Auth::user()->authorization)
            ...
            @case (1)
                {{Form::open(['method' => 'put', 'route' => ['orders.update', $store->id]]) }}  
                ...
                <div class="text-center">
                    ...
                    @foreach($products as $product)
                        ...
                        @if($product->orders->isEmpty()==False)

                            {{ Form::hidden('products[' . $loop->index . '][id]', $product->id, ['class' => 'form-control']) }}
                            {{ Form::number('products[' . $loop->index . '][quantity]', $product->orders[0]->quantity, ['class' => 'form-control text-right', 'min' => '0']) }}                        
                        @else
